...
const V = [
  ...
  ['one match consuming its slot source twice', 'bad-dup-source', r => r.errs.length > 0, /consumed twice by this match/],
  ['both sides resolving to the same team', 'bad-same-team', r => r.errs.length > 0, /same team/],
  ['slot cycle', 'cycle', r => r.errs.length > 0, /cycle/],
  ['two unfed knockout matches — not a single final', 'bad-two-finals', r => r.errs.length > 0, /exactly one championship final/],
