...
**`gb.js`** — the one CLI. `node gb.js` starts the match-day editor (edits run as single keys on the selected line — `s` score · `t` time · `v` venue · `w` walkover · `o` void; `:publish`, `:status`, `:use` for the rest — and every edit validates and commits itself); `node gb.js admin [slug]` starts the localhost admin UI — a browser page with the day's calendar grid (venues across, real time down, matches as slot-sized blocks), drag-to-reschedule (drops snap to the day's legal starts — the daemon computes them with the gate's own venue/player/feeder rules, so the preview and the write gate can't disagree), click-to-score/wo/void, a side-entry picker, a pending-changes panel (unpushed commits), undo (reset of the last unpushed commit) and a publish button (validate + push + surge) — the daemon reads site/ when it starts, so restart it if the repo changed in another terminal; `node gb.js validate [slug]` checks data without the editor; `node gb.js schedule <spec>` generates a tournament file from a spec (`specs/<slug>.json`); `node gb.js publish` ships `site/` to the domain in `site/CNAME` (requires the surge CLI: `npm install -g surge`); `node gb.js sim [slug]` rehearses a tournament in a browser: it copies `site/` to the gitignored `.sim/`, serves it with `Date.now` overridden to a sim clock (which drives the displayed board clock and statuses — never what's scoreable), and opens a keypress-driven editor whose sim keys move that clock and score the highlighted wave with random results through the same validation path — the venue board goes live in the browser, and nothing is ever committed. The commands live as modules under `src/`; `site/` stays the shipping surface.
